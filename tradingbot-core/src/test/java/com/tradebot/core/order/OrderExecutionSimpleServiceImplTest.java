...
public class OrderExecutionSimpleServiceImplTest<N> {
    ...
    private final TradingDecision<Object> tradingDecision1 = TradingDecision.builder()
        .signal(signal).instrument(gbpaud).tradeSource(SrcDecision.OTHER).limitPrice(1.855).stopPrice(2.21).units(1L)
    ...
    public void testPlaceOrders() throws ExecutionException, InterruptedException {
        ...
        OrderExecutionSimpleServiceImpl<Long, Long, Object> service = new OrderExecutionSimpleServiceImpl<>(
            orderManagementProvider,
            () -> accountInfoService.findAccountsToTrade().iterator().next(),
            orderExecutionServiceCallback);


        List<Order<Long>> tradingOrderList1 = service.createOrderListFromDecision(tradingDecision1);
        List<Order<Long>> tradingOrderList2 = service.createOrderListFromDecision(tradingDecision2);
    ...
    public void testOrderSubmissionIsInterrupted() throws ExecutionException, InterruptedException {
        ...
        List<Order<Long>> tradingOrderList = service.createOrderListFromDecision(tradingDecision1);
